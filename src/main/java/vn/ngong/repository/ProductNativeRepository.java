...
public class ProductNativeRepository {
    ...
    public List<ProductFilterDetail> findNewestSale(int limit) {
        try {
            Query query = entityManager.createNativeQuery("SELECT P.id, P.name, P.code, P.price, P.product_images AS image, IFNULL(S.rate * 100, 0) AS saleRate, IFNULL(S.`name`, '') AS saleName, IFNULL(S.sale_price, 0) AS salePrice, IFNULL(S.start_time, '') AS startTime, IFNULL(S.end_time, '') AS endTime " +
                    "FROM wp_products P " +
                    "INNER JOIN product_sales S ON P.Id = S.product_id AND S.`status` = 1 " +
                    "WHERE P.`status` = 1 " +
                    "GROUP BY P.code " +
