    ...
    public int getShipPrice(int cityCode, int districtCode, double weight, int totalPrice) {
        try {
            Query query = entityManager.createNativeQuery("SELECT s.id, s.price" +
                    " FROM ships s " +
                    " INNER JOIN ship_areas a ON a.code = s.ship_area_code" +
                    " WHERE s.`status` = 1" +
                    " AND a.`status` = 1" +
                    " AND a.city_code = " + cityCode +
                    " AND (IFNULL(a.district_code, '') = '' OR a.district_code = " + districtCode + ")" +
                    " AND ((s.start_kg = 0 AND s.end_kg = 0) OR (s.end_kg = 0 AND s.start_kg <= " + weight + ") OR (" + weight + " BETWEEN s.start_kg AND s.end_kg))" +
                    " AND ((s.start_money = 0 AND s.end_money = 0) OR (s.end_money = 0 AND s.start_money <= " + totalPrice + ") OR (" + totalPrice + " BETWEEN s.start_money AND s.end_money))" +
                    " LIMIT 1");
